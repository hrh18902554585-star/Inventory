...
class BatchQueryApp:
    def __init__(self, root):
        self.root = root
        self.root.title("菜鸟充值订单核算系统 (全店多表汇总)")
        ...
        self.link_file_path = ttk.StringVar()
        self.promo_file_path = ttk.StringVar()
        self.promo_file_paths = []
        self.subsidy_file_path = ttk.StringVar()
        self.sort_file_path = ttk.StringVar()
        
        self.cookie_var = ttk.StringVar(value=DEFAULT_COOKIE)
        self.exclude_product_ids_var = ttk.StringVar()
        self.status_var = ttk.StringVar(value="准备就绪")
        self.progress_var = ttk.DoubleVar()
        self.thread_count = ttk.IntVar(value=6)
        self.enable_tax_query = ttk.BooleanVar(value=True) # 新增查税运开关
        
        self.config_file = "cookie.txt"
        self.exclude_ids_file = "exclude_ids.txt"
        self.paths_file = "paths_config.txt"
        self.tax_cache_file = "tax_cache.json"
        self.tax_cache = {}
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, "r", encoding="utf-8") as f:
                    cookie = f.read().strip()
                    if cookie: self.cookie_var.set(cookie)
            except: pass
        if os.path.exists(self.exclude_ids_file):
            try:
                with open(self.exclude_ids_file, "r", encoding="utf-8") as f:
                    exclude_ids = f.read().strip()
                    if exclude_ids: self.exclude_product_ids_var.set(exclude_ids)
            except: pass
        if os.path.exists(self.paths_file):
            try:
                with open(self.paths_file, "r", encoding="utf-8") as f:
                    lines = f.read().splitlines()
                    if len(lines) >= 1 and lines[0]: self.link_file_path.set(lines[0])
                    if len(lines) >= 2 and lines[1]:
                        self.promo_file_paths = lines[1].split("|")
                        self.promo_file_path.set(" | ".join([os.path.basename(p) for p in self.promo_file_paths]))
                    if len(lines) >= 3 and lines[2]: self.subsidy_file_path.set(lines[2])
                    if len(lines) >= 4 and lines[3]: self.sort_file_path.set(lines[3])
            except: pass
            
        if os.path.exists(self.tax_cache_file):
            try:
                with open(self.tax_cache_file, "r", encoding="utf-8") as f:
                    self.tax_cache = json.load(f)
            except: pass
            
        # 缓存回落：如果没选产品链接表，自动找同目录下的
        if not self.link_file_path.get().strip():
            for c in ["产品链接汇总表.xlsx", "产品链接汇总表_模版.xlsx"]:
                if os.path.exists(c):
                    self.link_file_path.set(os.path.abspath(c))
                    break
                    
        # 缓存回落：官补映射表
        if not self.subsidy_file_path.get().strip():
            if os.path.exists("官补映射表.xlsx"):
                self.subsidy_file_path.set(os.path.abspath("官补映射表.xlsx"))
                
        # 缓存回落：产品排序表
        if not self.sort_file_path.get().strip():
            if os.path.exists("产品汇总表.xlsx"):
                self.sort_file_path.set(os.path.abspath("产品汇总表.xlsx"))

    def save_tax_cache(self):
        if getattr(self, 'tax_cache_dirty', False):
            try:
                with open(self.tax_cache_file, "w", encoding="utf-8") as f:
                    json.dump(self.tax_cache, f, ensure_ascii=False, indent=2)
                self.tax_cache_dirty = False
            except Exception as e:
                self.log(f"保存税运缓存失败: {e}")

    def save_config(self):
        try:
            with open(self.config_file, "w", encoding="utf-8") as f: f.write(self.cookie_var.get().strip())
        except: pass
        try:
            with open(self.exclude_ids_file, "w", encoding="utf-8") as f: f.write(self.exclude_product_ids_var.get().strip())
        except: pass
        try:
            with open(self.paths_file, "w", encoding="utf-8") as f: 
                f.write(f"{self.link_file_path.get().strip()}\n")
                f.write("|".join(self.promo_file_paths) + "\n")
                f.write(f"{self.subsidy_file_path.get().strip()}\n")
                f.write(f"{self.sort_file_path.get().strip()}\n")
        except: pass
        
    def create_widgets(self):
        # 整体采用更现代的 padding 和布局
        ...
        # 文件选择
        file_frame = ttk.Labelframe(top_frame, text=" 第一步：选择业务源数据文件 (支持多选) ", padding=15, bootstyle=PRIMARY)
        file_frame.pack(fill=X, pady=5)
        ttk.Entry(file_frame, textvariable=self.file_path, width=70).pack(side=LEFT, padx=(0, 10), fill=X, expand=YES)
        ttk.Button(file_frame, text="浏览文件", command=self.browse_file, bootstyle=OUTLINE).pack(side=LEFT)
        
        # 配置表选择
        cfg_frame = ttk.Labelframe(top_frame, text=" 第二步：选择配置表 (必填) ", padding=15, bootstyle=INFO)
        cfg_frame.pack(fill=X, pady=5)
        
        f1 = ttk.Frame(cfg_frame)
        f1.pack(fill=X, pady=(0, 10))
        ttk.Label(f1, text="产品链接汇总表:", width=16, anchor="e").pack(side=LEFT, padx=(0, 10))
        ttk.Entry(f1, textvariable=self.link_file_path).pack(side=LEFT, fill=X, expand=YES, padx=(0, 10))
        ttk.Button(f1, text="浏览文件", command=lambda: self.browse_single_file(self.link_file_path), bootstyle=OUTLINE).pack(side=LEFT)

        f2 = ttk.Frame(cfg_frame)
        f2.pack(fill=X, pady=(0, 10))
        ttk.Label(f2, text="推广报表(多选):", width=16, anchor="e").pack(side=LEFT, padx=(0, 10))
        ttk.Entry(f2, textvariable=self.promo_file_path).pack(side=LEFT, fill=X, expand=YES, padx=(0, 10))
        ttk.Button(f2, text="浏览文件", command=self.browse_promo_files, bootstyle=OUTLINE).pack(side=LEFT)
        
        f3 = ttk.Frame(cfg_frame)
        f3.pack(fill=X, pady=(0, 10))
        ttk.Label(f3, text="官补映射表:", width=16, anchor="e").pack(side=LEFT, padx=(0, 10))
        ttk.Entry(f3, textvariable=self.subsidy_file_path).pack(side=LEFT, fill=X, expand=YES, padx=(0, 10))
        ttk.Button(f3, text="浏览文件", command=lambda: self.browse_single_file(self.subsidy_file_path), bootstyle=OUTLINE).pack(side=LEFT)
        
        f4 = ttk.Frame(cfg_frame)
        f4.pack(fill=X)
        ttk.Label(f4, text="产品排序表(可选):", width=16, anchor="e").pack(side=LEFT, padx=(0, 10))
        ttk.Entry(f4, textvariable=self.sort_file_path).pack(side=LEFT, fill=X, expand=YES, padx=(0, 10))
        ttk.Button(f4, text="浏览文件", command=lambda: self.browse_single_file(self.sort_file_path), bootstyle=OUTLINE).pack(side=LEFT)
        
        # Cookie 设置
        cookie_frame = ttk.Labelframe(top_frame, text=" 第三步：设置请求 Cookie ", padding=15, bootstyle=SECONDARY)
        cookie_frame.pack(fill=X, pady=5)
        ttk.Label(cookie_frame, text="Cookie:").pack(side=LEFT, padx=(0, 10))
        ttk.Entry(cookie_frame, textvariable=self.cookie_var).pack(side=LEFT, fill=X, expand=YES)
        
        # 配置区域
        config_frame = ttk.Labelframe(top_frame, text=" 第四步：其他配置参数 ", padding=15, bootstyle=DARK)
        config_frame.pack(fill=X, pady=5)
        ttk.Label(config_frame, text="并发线程数:").pack(side=LEFT, padx=(0, 5))
        ttk.Spinbox(config_frame, from_=1, to=20, textvariable=self.thread_count, width=5).pack(side=LEFT, padx=(0, 20))
        
        ttk.Checkbutton(config_frame, text="开启查税运", variable=self.enable_tax_query, bootstyle="round-toggle").pack(side=LEFT, padx=(0, 20))
        
        ttk.Label(config_frame, text="排除商品ID (英文逗号分隔):").pack(side=LEFT, padx=(0, 5))
        ttk.Entry(config_frame, textvariable=self.exclude_product_ids_var).pack(side=LEFT, fill=X, expand=YES)
        
        # 操作按钮
        action_frame = ttk.Frame(main_frame, padding=(0, 10))
        action_frame.pack(fill=X)
        self.start_btn = ttk.Button(action_frame, text="🚀 开始批量处理", command=self.start_processing_thread, bootstyle=SUCCESS, padding=10)
        ...
        status_sub_frame.pack(fill=X)
        ttk.Label(status_sub_frame, textvariable=self.status_var, font=("Helvetica", 10, "bold"), bootstyle=PRIMARY).pack(side=LEFT)
        
        self.result_link_label = ttk.Label(status_sub_frame, text="", font=("Helvetica", 10, "underline"), cursor="hand2", foreground="#0056b3")
        self.result_link_label.pack(side=RIGHT)
        self.result_link_label.bind("<Button-1>", self.open_result_file)
        self.result_link_path = ""
        
        # 日志区域
        log_frame = ttk.Labelframe(main_frame, text=" 运行日志 ", padding=10)
        log_frame.pack(fill=BOTH, expand=YES, pady=5)
        self.log_text = tk.Text(log_frame, height=10, font=("Consolas", 9), bg="#f8f9fa", relief=tk.FLAT)
    ...
    def browse_file(self):
        ...
        if filenames:
            ...
            display_text = " | ".join(self.file_paths[:3]) + (f" ... 共{len(self.file_paths)}个文件" if len(self.file_paths) > 3 else "")
            self.file_path.set(display_text)
            self.log(f"已选择 {len(self.file_paths)} 个源文件")

    def browse_single_file(self, var):
        filename = filedialog.askopenfilename(filetypes=[("Excel/CSV files", "*.xlsx *.csv")])
        if filename:
            var.set(filename)

    def browse_promo_files(self):
        filenames = filedialog.askopenfilenames(filetypes=[("Excel/CSV files", "*.xlsx *.csv")])
        if filenames:
            self.promo_file_paths = list(filenames)
            display_text = " | ".join([os.path.basename(p) for p in self.promo_file_paths[:3]]) + (f" ... 共{len(self.promo_file_paths)}个文件" if len(self.promo_file_paths) > 3 else "")
            self.promo_file_path.set(display_text)
            self.log(f"已选择 {len(self.promo_file_paths)} 个推广报表")
    ...
    def load_cost_map(self):
        candidates = ["成本表.xlsx", "成本表.成本表.xlsx"]
        cost_file_path = ""
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if not row or sku_col >= len(row) or row[sku_col] is None: continue
            sku_text = str(row[sku_col]).strip()
            if not sku_text: continue
            cost_value = row[cost_col] if cost_col < len(row) else ""
            
            # G列是index 6, 如果表头找不到日期列，尝试读取第7列
            date_val = ""
    ...
    def load_link_map(self):
        link_path = self.link_file_path.get().strip()
        if not link_path or not os.path.exists(link_path):
            raise Exception("无效的产品链接汇总表路径")
        ...
        if store_col is None or prod_col is None or id_col is None:
            wb.close()
            raise Exception("产品链接汇总表缺少表头: 店铺名称 | 产品名称 | 商品ID")
            
        link_map = {} # store_name -> {product_id: product_name}
        for row in ws.iter_rows(min_row=2, values_only=True):
            if not row or store_col >= len(row) or row[store_col] is None: continue
    ...
    def load_promo_map(self):
        if not self.promo_file_paths:
            raise Exception("请选择至少一个推广报表文件")
            
        promo_map = {} # store_name -> {product_id: {"total": 0.0, "daily": {}}}
        total_loaded = 0
        
        for promo_path in self.promo_file_paths:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                if not row or id_col >= len(row) or row[id_col] is None: continue
    ...
    def load_subsidy_map(self):
        subsidy_path = self.subsidy_file_path.get().strip()
        ...
        subsidy_map = {} # link_id -> { sku: [(start_date, end_date, subsidy_amount)] }
        for row in ws.iter_rows(min_row=2, values_only=True):
            if not row or link_id_col >= len(row) or row[link_id_col] is None: continue
    ...
    def load_sort_list(self):
        sort_path = self.sort_file_path.get().strip()
    ...
    def extract_store_name(self, filename):
        ...
        for s in known_stores:
            if s in base: return s
        # 默认匹配第一个识别出的店铺，如果都没有返回未知
        return "未知店铺"

    def extract_date_from_filename(self, filename):
        base = os.path.basename(filename)
        
        # 尝试匹配 20260526 或 2026-05-26 或 2026年05月26日 等完整格式
        match = re.search(r'(20\d{2})[-年]?(\d{1,2})[-月]?(\d{1,2})日?', base)
        if match:
            year, month, day = match.groups()
            return f"{year}年{int(month):02d}月{int(day):02d}日"
            
        # 尝试匹配 26-5-26 这种简写格式 (默认加上 20 前缀)
    ...
    def read_source_file(self, filename):
        ...
        if is_csv:
            ...
            for enc in encodings:
                try:
                    with open(filename, 'r', encoding=enc) as f: f.read()
                    encoding = enc
                    break
                except UnicodeDecodeError: continue
            if not encoding: raise Exception(f"无法识别文件编码: {filename}")
            ...
            for i, row in enumerate(ws.iter_rows(values_only=True)):
                if i == 0: headers = [str(c or "") for c in row]
                else: rows.append([c for c in row])
            wb.close()
        return headers, rows

    def start_processing_thread(self):
        if not self.file_paths:
            messagebox.showwarning("警告", "请选择业务源数据文件")
            return
        if not self.cookie_var.get().strip():
            messagebox.showwarning("警告", "请输入Cookie")
            return
        if not self.link_file_path.get().strip() or not self.promo_file_paths:
            messagebox.showwarning("警告", "请选择配置表 (产品链接汇总表 和 推广报表)")
            return
            
        self.result_link_label.config(text="")
        self.result_link_path = ""
    ...
    def open_result_file(self, event):
        if self.result_link_path and os.path.exists(self.result_link_path):
            try:
                os.startfile(self.result_link_path)
            except Exception as e:
                self.log(f"打开文件失败: {e}")

    def process_row_api(self, task, cookie, enable_tax=True, tax_cache=None):
        ...
        if tax_cache is not None and str(order_no).strip() in tax_cache:
            cached = tax_cache[str(order_no).strip()]
        ...
        else: msg = f"失败: {result.get('error') if result else '未知错误'}"
    ...
    def process_all_files(self):
        try:
            cost_map, baby_powder_codes = self.load_cost_map()
            link_map = self.load_link_map()
            promo_map = self.load_promo_map()
            subsidy_map = self.load_subsidy_map()
            order_list = self.load_sort_list()
            excluded_ids = self.parse_excluded_product_ids()
            
            all_date_store_data = {} # date_str -> store_name -> {"headers": [], "rows": [], "results": {}}
            all_tasks = []
            ...
            for filepath in self.file_paths:
                ...
                file_date = self.extract_date_from_filename(filepath) or datetime.now().strftime("%Y年%m月%d日")
                ...
                if None in (order_no_idx, status_idx, sku_idx, qty_idx, receive_idx):
                    self.log(f"跳过文件 {os.path.basename(filepath)}: 缺少必要表头")
                    continue
                ...
                for i, row in enumerate(rows):
                    ...
                    if order_status not in allowed_status: continue
                    if product_id and product_id in excluded_ids: continue
            ...
            self.log(f"总计需要查询 {total_tasks} 个有效订单")
            
            global_fee_names = set()
            processed_count = 0
            cookie = self.cookie_var.get().strip()
            thread_num = self.thread_count.get()
            enable_tax = self.enable_tax_query.get()
            
            # 阶段 2：并发查询 API
            if total_tasks > 0:
                with ThreadPoolExecutor(max_workers=thread_num) as executor:
                    futures = {executor.submit(self.process_row_api, task, cookie, enable_tax, self.tax_cache): task for task in all_tasks}
                    for future in as_completed(futures):
                        try:
                            ...
                            for fn in fee_details.keys(): global_fee_names.add(fn)
                            ...
                            # 基础毛利 = 实收 - 综合成本 - 平台扣点 + 官补金额
                            # 推广费属于产品/店铺级，后面汇总时再减去
                        ...
                        processed_count += 1
                        self.progress_var.set((processed_count / total_tasks) * 100)
                        self.status_var.set(f"查询进度: {processed_count}/{total_tasks}")
                        if processed_count % 20 == 0 or processed_count == total_tasks:
                            self.log(f"已查询 {processed_count}/{total_tasks}")
                            
            # 如果存在多天数据，生成一份合并汇总的数据
            ...
            for date_str, all_store_data in all_date_store_data.items():
                ...
                yellow_fill = PatternFill(start_color="FFFFE0", end_color="FFFFE0", fill_type="solid") # 浅黄色底色
                gray_side = Side(style="thin", color="FFE7E6E6")
                gray_fill = PatternFill(start_color="FFF2F2F2", end_color="FFF2F2F2", fill_type="solid") # 浅灰色空行
                gray_border = Border(left=gray_side, right=gray_side, top=gray_side, bottom=gray_side)
                
                # 全店汇总数据结构
                # { product_name: { link_id: {sales, orders, promo, tax} } }
                global_agg = {}
                global_store_totals = {}
                store_agg_map = {} # store_name -> store_agg
                
                # 分类收集各个Sheet，以便最后按顺序排列
                sheet_groups = {
                    "raw": [],
                    "proc": [],
                    "detail": []
                ...
                for store_name, data in all_store_data.items():
                        ...
                        for val in row:
                            if isinstance(val, str):
                                ...
                                if re.match(r'^-?\d+\.\d+$', v_strip):
                                    ...
                                    # 注意：像订单号等长数字可能转为整数会在Excel中显示科学计数法或丢失精度
                                    # 因此我们只转换合理的数值范围，例如不超过15位数字
                                    if len(v_strip) <= 15:
                    ...
                    store_agg = {} # product_name -> link_id -> stats
                    ...
                    for row_dict in data["rows"]:
                        ...
                        for col_idx in range(1, len(proc_headers) + 1):
                            ...
                            cell.fill = row_fill
                            if row_fill == white_fill: cell.border = gray_border
                            
                        # 聚合数据 (按产品名称和链接ID)
                        p_id = res.get("product_id", "")
                        p_name = store_link_map.get(p_id, "未知产品")
                        
                        if p_name not in store_agg: store_agg[p_name] = {}
                        ...
                        store_agg[p_name][p_id]["orders"] += 1 # 按行计为订单数
                        store_agg[p_name][p_id]["items"] += items_count # 累加商品数量
                        ...
                        # 全店聚合
                        if p_name not in global_agg: global_agg[p_name] = {}
                ...
                # 在所有店铺数据遍历完毕后，对全店汇总的 global_agg 进行排序
                ...
                for store_name, data in all_store_data.items():
                    ...
                    sheet_groups["proc"].append(ws_promo) # 将其跟在处理表之后
                    ...
                    for p_name, global_links in sorted_global_agg:
                        ...
                        if end_row > start_row:
                            ws_detail.merge_cells(start_row=start_row, end_row=end_row, start_column=1, end_column=1) # 店铺名称
                            ws_detail.merge_cells(start_row=start_row, end_row=end_row, start_column=2, end_column=2) # 产品名称
                ...
                for p_name, links in sorted_global_agg:
                    ...
                    for idx, p_id in enumerate(sorted_p_ids):
                        ...
                        # 在全店汇总表中，我们将所有店铺处理表通过加号连接起来的 SUMIFS
                        sales_formula_parts = []
                    ...
                    if end_row > start_row:
                        ws_global.merge_cells(start_row=start_row, end_row=end_row, start_column=1, end_column=1) # 产品名称
                ...
                title_cell.fill = PatternFill("solid", fgColor="D9EAD3") # 浅绿色标题
                ...
                for row_idx in range(header_row, ws_global.max_row + 1):
                    for col_idx in range(1, 12):
                        ...
                        if row_idx == header_row:
                            cell.fill = PatternFill("solid", fgColor="D9EAD3") # 浅绿色表头
                        else:
                            cell.fill = yellow_fill
                        cell.border = gray_border
                ...
                for p_name, links in sorted_global_agg:
                    # 获取全店汇总表中对应的行，以便直接引用
                    # 我们可以通过查找刚才写入的数据范围，或者使用SUMIFS引用处理表。
                    # 为了更彻底的动态，这里也用SUMIFS跨表查询各处理表。
                    plan_sales_parts = []
                    plan_orders_parts = []
                    plan_items_parts = []
                    plan_tax_parts = []
                    ...
                    for s_name in all_store_data.keys():
                        ...
                        s_n_orig = len(s_headers)
                        s_items_col = get_column_letter(s_n_orig + 1) # 实际商品数量
                        s_tax_col = get_column_letter(s_n_orig + 2) # 税运
                        s_all_cost_col = get_column_letter(s_n_orig + 5) # 综合成本
                        s_profit_col = get_column_letter(s_n_orig + 7) # 毛利润列在总和税运, 商品成本, 订单成本, 综合成本, 平台扣点, 毛利润
                        
                        promo_val_col = "C" if "至" in date_str else "B"
                        
                        for p_id in links.keys():
                            if p_id not in store_link_map.get(s_name, {}):
                                continue # 只有这个产品在这个店铺里卖了，才添加 SUMIFS 引用
                ...
                for target_p in target_products:
                    ...
                    for single_date in sorted(list(all_date_store_data.keys())):
                        ...
                        for s_name, s_info in s_date_data.items():
                            ...
                            s_date_col = get_column_letter(s_n_orig + 8) # 归属日期
                ...
                for sheet_group in [sheet_groups["detail"], sheets_for_width]:
                    for ws in sheet_group:
                        for col in ws.columns:
                            ...
                            for cell in col:
                                if cell.value:
                                    # 计算文字长度（中文字符算作1.5到2个宽度）
                                    val_str = str(cell.value)
                                    # 简单估算：中文字符较多，每个字符按 1.8 宽度算
                ...
                ordered_sheets.append(ws_plan)  # 将销售计划登记表追加到最后
                ordered_sheets.extend(daily_sheets_to_add) # 追加每日数据表
                
                # 把现有的 sheet 按照排序后的顺序重新赋给 wb._sheets
            ...
            # 设置可点击链接
            self.result_link_path = os.path.abspath(output_dir)
            self.root.after(0, lambda: self.result_link_label.config(text="👉 点击打开输出文件夹"))
            
            self.status_var.set("处理完成！")
            self.log("任务完成！所有汇总文件已生成。")
            self.root.after(0, lambda: self.start_btn.config(state="normal", text="🚀 开始批量处理"))
...
if __name__ == "__main__":
    root = ttk.Window(themename="cosmo") # 使用 cosmo 主题，类似 Element UI 风格
